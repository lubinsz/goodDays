...
static NSString *_titleHint = @"祝健康快乐";
...
#if 1
    [self.timeButton addTarget:self action:@selector(onDateButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
#endif
#if 0
    [self.dateButton addTarget:self action:@selector(onTimeButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setupBadges
{
    ...
    if (self.event) {
        ...
        //[_badges addObject:[NSString stringWithFormat:@"%@日",[self.event.date getTypeString]]];
...
- (BOOL)validateNewEventValue
{
    ...
    if (!_pickedHasReminder) {
        ...
        _pickedMinutesBefore = [NSNumber numberWithInteger:60]; //1 hour
        //[self updateReminderBadge];
    }
#endif
    if (!hasTitle) {
    ...
    if (!hasTime)
    {
        _pickedHour = [NSNumber numberWithInt: 9];;                // 9:00 AM bblu9
        _pickedMinute = [NSNumber numberWithInt: 0];;
...
- (Event *)createNewEvent
{
    ...
    //added by bblu
    //NSLog(@"bblu createNewEvent：%@,%@,%@",date,time,reminder);
...
- (void)updateCountdownLabel
{
    ...
    [UIView animateWithDuration:.1
                     animations:^{
                         if (next) {
                             if (expired) {
                                 self.countdownLabel.text = @"Expired111";
                                 ...
                                 self.countdownLabel.text = [NSString stringWithFormat:@"%@ %@", @"还剩",
...
- (IBAction)onCreateTapped:(id)sender
{
    ...
    //NSLog(@"bblu onCreateTapped...");
    if ([self validateNewEventValue]) {
        Event *event = [self createNewEvent];
        //important, bblu, add the notification here.
        //maybe we should call this 3 times
        // 8:00 each days
...
- (IBAction)onDeleteTapped:(id)sender
{
    [GoogleAnalyticsHelper trackEventWithCategory:GA_CATEGORY_UI
                                           action:GA_ACTION_DELETE_BUTTON
                                            label:nil
                                            value:nil];
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"删除该日期?"
                                                    message:nil
                                                   delegate:self
...
- (IBAction)onDateButtonTapped:(id)sender
{
    ...
    if (!_calendarPickerShown)
        ...
        [self hideControlContainer];
#else
    //NSLog(@"bblu quickCalendarPickerViewDidSelectPickOther ");
    //[self hideControlContainer];
    ...
    if (self.event) {
        ...
        //NSLog(@"bblu2: %d,%d,%d,%d,%d...",vc.type,vc.day,vc.weekday,vc.month,vc.year);
    } else {
        _pickedType = [NSNumber numberWithInteger:RBZEventYearly];
        vc.type = _pickedType;
        vc.day = _pickedDay;
        vc.weekday = _pickedWeekday;
        vc.month = _pickedMonth;
        vc.year = _pickedYear;
        //NSLog(@"bblu3: %d,%d,%d,%d,%d...",vc.type,vc.day,vc.weekday,vc.month,vc.year);
    }
    [self presentViewController:vc animated:YES completion:nil];
#endif
}
...
- (void)displayEventTimeHint
{
    ...
    if (![defaults boolForKey:DEFAULTS_TIME_HINT_SHOWN]) {
        ...
        //NSLog(@"bblu displayEventTimeHint");
...
{
#if 0
    NSLog(@"bblu displayEventDateHint");
    ...
    if (![defaults boolForKey:DEFAULTS_DATE_HINT_SHOWN]) {
        ...
        NSLog(@"bblu dateButton:%@",self.dateButton.center.x);
