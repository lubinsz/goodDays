...
- (void)updateHeadUpDisplay
{
    ...
    //NSLog(@"bblu updateHeadUpDisplay");
...
#else
    if ( ([_todayEvents count] == 0) && ([_tomorrowEvents count] !=0))
    {
        //NSLog(@"bblu 1212121212");
        _upcomingEventFromTomorrow = YES;
        _upcomingEvent = _tomorrowEvents[0];
    }
    else
...
#endif
    //NSLog(@"bblu 111:%d,%d",[old.date.day integerValue],[_upcomingEvent.date.day integerValue]);
//**************
#if 1
    ...
    else
    {
        ...
        //NSDate *date2=[dateFromString(<#NSString *value#>, <#NSString *format#>)
        //[event getBetweenDays:today];
        //NSLog(@"bblu test1111");
        //_pickedBetweenDays = [event getBetweenDays:today];
        //str = [_upcomingEvent.time getTimeString];
        str = [NSString stringWithFormat:@"%dDays",[_upcomingEvent getBetweenDays:today]];
        
        NSLog(@"bblu str is %@",str);
    }
#else
    NSDate *today = _displayDate;
    ...
    //_pickedBetweenDays = [event getBetweenDays:today];
    //str = [_upcomingEvent.time getTimeString];
    NSInteger days = [_upcomingEvent getBetweenDays:today];
    
    if ( days == 0 )
    {
        str = [NSString stringWithFormat:@"今天"];
    }
    else
    {
        str = [NSString stringWithFormat:@"%d 天", days];
    }
    //NSLog(@"bblu str is %@",str);
    self.hudTimeLabel.backgroundColor = [UIColor clearColor];
#endif
    [UIView animateWithDuration:.2
                     animations:^{
                         if (_upcomingEvent) {
                             //NSLog(@"bblu has _upcomingEvent...");
                             _dummyHudTimeLabel.text = str;
                             [_dummyHudTimeLabel sizeToFit];
                             CGFloat width = _dummyHudTimeLabel.frame.size.width + 12.0;
                             
                             self.hudTitleLabel.text = [NSString stringWithFormat:@"%@(%@/%@)",[_upcomingEvent title],[_upcomingEvent.date month],[_upcomingEvent.date day]];
...
//NSDate *date2=[dateFromString(<#NSString *value#>, <#NSString *format#>)
    ...
    //_pickedBetweenDays = [event getBetweenDays:today];
    //NSLog(@"bblu _pickedBetweenDays: %d...",_pickedBetweenDays);
   // event.days = 111;
   // NSLog(@"bblu days:%d",event.days);
#if 0
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //NSLog(@"bblu cellforrow....");
    NSDate *today = _displayDate;

    //NSTimeInterval time = [ ev.date timeIntervalSinceDate:today];
    //[ev.date.day ]
    
    cell.titleLabel.text = ev.title;
    cell.titleLabel.text =[NSString stringWithFormat:@"%@(%@/%@)",ev.title,[ev.date month],[ev.date day]];
  ...
  //  [event getBetweenDays:today];
   // NSString *datestring = [NSString stringWithFormat:@"%d-%d-%d",destYear,[self.date.month integerValue],[self.date.day integerValue]];
    NSInteger days = [ev getBetweenDays:today];
    ...
    {
        cell.timeLabel.text = [NSString stringWithFormat:@"%d 天",days];
    }
...
- (void)updateAnchorPointAndOffset:(CGPoint)anchorPoint view:(UIView*)view {
    ...
    //NSLog(@"bblu updatePoint:%@....",xOffset * view.frame.size.width);
